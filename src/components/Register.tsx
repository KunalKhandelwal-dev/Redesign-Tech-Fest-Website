...
;
  ...
  const showFloatingToast = (msg: string) => {
    ...
    toast.style.background = "rgba(0, 180, 255, 0.15)";
    toast.style.color = "#00e6ff";
    toast.style.padding = "10px 22px";
    toast.style.borderRadius = "10px";
    toast.style.fontFamily = "Orbitron, sans-serif";
    toast.style.border = "1px solid rgba(0, 255, 255, 0.3)";
    toast.style.boxShadow = "0 0 12px rgba(0,255,255,0.3)";
  ...
  const toggleEvent = (label: string, isExternal = false) => {
    ...
    setFormData((prev) => {
      if (currentlySelected) {
        ...
        const newTeamType =
          newEventType.some((e) => eventInfo[e]?.type === "team") ? "team" : "individual";
        if (info.type === "team") {
          return {
            ...prev,
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          // Convert array fields like teamMembers, eventType to comma-separated string
      ...
      if (formData.paymentReceipt instanceof File) {
        form.append("paymentReceipt", formData.paymentReceipt);
      }
  ...
  ----------------------------- */
  const isTeamEventSelected = formData.eventType.some(
    (e) => eventInfo[e]?.type === "team"
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "eventSelected",
      handleEventSelect as EventListener
    );
    return () =>
                ...
                <div className="p-3 border border-cyan-500/20 rounded-xl bg-[#0f1724] space-y-3">
                  ...
                  <div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        const max = Math.max(
                          ...formData.eventType.map((e) => eventInfo[e]?.maxTeam || 1)
                        );
                ...
                <div className="p-3 rounded-xl border border-cyan-500/20 bg-[#0f1724] text-center space-y-3">
                  ...
                  <div className="text-left">
                    ...
                    <input
                      ...
                      className="block w-full text-sm text-gray-300 bg-[#0f1724] border border-cyan-500/20 rounded-lg p-2"
                    />
                    {formData.paymentReceipt && (
                      <p className="text-cyan-400 text-sm mt-2">✅ {formData.paymentReceipt.name}</p>
                    )}
                    {errors.paymentReceipt && (
                      <p className="text-red-400 text-sm mt-1">{errors.paymentReceipt}</p>
                    )}
                  </div>
                </div>
              )}
